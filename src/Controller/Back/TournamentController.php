...
class TournamentController extends AbstractController
{
    ...
    public function endTrial(Request $request, Tournament $tournament, Trial $trial, EntityManagerInterface $entityManager, TrialRepository $trialRepository, TrialService $ts, BetService $betService): Response
    {
        ...
        $winner = $entityManager->getRepository(User::class)->findOneBy(['id' => $request->request->get('fighter')]);
        $next = $trial->getNextTrial();
        if (count($trials) == 1 && in_array($trial, $trials)) {
            $tournament->setStep($tournament->getStep() + 1);
            if (!$next) {
